...
class AlertParameters(BaseModel):
    ...
    AWIPSidentifier: List[str] = Field(
        description="AWIPS (Advanced Weather Interactive Processing System) identifier"
    )
    WMOidentifier: List[str] = Field(
    ...
    EAS_ORG: List[str] = Field(
        alias="EAS-ORG", description="Emergency Alert System organization"
    )
