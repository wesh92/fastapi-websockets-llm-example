    ...
    try:
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Connection {connection_id}: Received raw message: {raw_message}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Connection {connection_id}: Successfully parsed message"
                    )
                ...
                async for response in openrouter_service.process_request(data):
                    logger.debug(
                        f"Connection {connection_id}: Sending response: {response}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Connection {connection_id}: Failed to send error message: {str(send_error)}"
                    )

    except Exception as e:
        logger.error(
            f"Connection {connection_id}: Fatal error: {str(e)}", exc_info=True
        )
