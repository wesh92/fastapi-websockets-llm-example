...
from .openrouter_models import WebSocketStateModel
...
class OpenRouterService:
    ...
    def _parse_sse_line(self, line: str) -> Optional[Dict[str, Any]]:
        ...
        if line.startswith(": "):
            ...
            return {"type": "processing", "message": processing_msg}
    ...
    async def process_request(
        ...
        try:
            ...
            async with self._session.post(
                self.config.api_url, headers=self._get_headers(), json=payload
            ) as response:
                logger.info(
                    f"Request {current_request_id}: Got response with status {response.status}"
                )

                if response.status != 200:
                    error_text = await response.text()
                    logger.error(
                        f"Request {current_request_id}: API error: {error_text}"
                    )
                    yield {
                        "type": "error",
                ...
                async for line in response.content:
                    ...
                    if parsed:
                        if parsed["type"] == "completion":
                            ...
                            delta_content = parsed["data"]["choices"][0]["delta"].get(
                                "content", ""
                            )
                            full_response += delta_content
                            final_metadata = parsed[
                                "data"
                            ]  # Keep latest metadata in case of updates.

                            # Check for finish reason
                            finish_reason = parsed["data"]["choices"][0].get(
                                "finish_reason"
                            )
                            if finish_reason:
                            ...
                            logger.warning(
                                f"Unexpected response type: {parsed['type']}"
                            )
            ...
            logger.error(
                f"Request {current_request_id}: Fatal error: {str(e)}", exc_info=True
            )
...
class WebSocketStateService:
    ...
    def create_connection(self, connection_id: str) -> WebSocketStateModel:
        ...
        state = WebSocketStateModel(
            connection_id=connection_id, connected_at=now, last_activity=now
        )
